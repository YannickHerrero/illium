    ...
    while let Some((path, depth)) = pending.pop() {
        ...
        if m.is_dir() {
            ...
            for entry in fs::read_dir(&path).map_err(|e| e.to_string())? {
                ...
                if !winarchy_theme::pack::plain_name(&name)
...
fn active_theme(home: &Path) -> Result<String> {
    let global: toml::Value =
        toml::from_str(&text(&home.join("winarchy.toml"))?).map_err(|e| e.to_string())?;
    global
        .get("theme")
        .and_then(|v| v.as_str())
        .map(str::to_owned)
    ...
    for kind in [Kind::Applet, Kind::Theme] {
        ...
        for (index, entry) in entries.enumerate() {
            ...
            let validation = metadata(&path).and_then(|_| match kind {
                ...
                Kind::Theme => winarchy_theme::Theme::load(home, &id).map(|_| ()),
            });
...
fn validate_applet(home: &Path, id: &str) -> Result<()> {
    ...
    if applet.manifest.provider.is_none() {
        if let Some(command) = &applet.manifest.command {
            ...
            if script
                .split('/')
                .any(|part| !winarchy_theme::pack::plain_name(part) || windows_reserved(part))
            {
...
fn prepare(stage: &Path, source: &Path) -> Result<Package> {
    ...
    match package.kind {
        ...
        Kind::Theme => {
            ...
            copy(&payload, &target)?;
            winarchy_theme::pack::install(stage, &target)?;
        }
    }
    Ok(package)
}
