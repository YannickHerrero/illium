...
fn dynamic_wallpaper_lifecycle_is_latest_wins_and_failure_safe() {
    ...
    let temp =
        Temp(std::env::temp_dir().join(format!("winarchy-dynamic-ui-{}", std::process::id())));
    ...
    assert_eq!(
        state,
        std::fs::read(temp.0.join(winarchy_theme::dynamic::FILE)).unwrap()
    );
    ...
    assert_eq!(shell.wallpaper.as_deref(), Some("b.png"));
    assert_eq!(
        winarchy_theme::Theme::effective(&temp.0)
            .unwrap()
            .mode
            .as_deref(),
        Some("light")
    ...
    settle(&mut shell);
    assert_eq!(shell.wallpaper.as_deref(), Some("a.png"));
    let a_palette = winarchy_theme::Theme::effective(&temp.0).unwrap();
    ...
    assert_ne!(
        a_palette.accent,
        winarchy_theme::Theme::effective(&temp.0).unwrap().accent
    );
    ...
    std::fs::write(temp.0.join(winarchy_theme::dynamic::FILE), "broken").unwrap();
    let recovered = Config::load(&temp.0).unwrap();
    assert_eq!(
        recovered.theme,
        winarchy_theme::Theme::load(&temp.0, "dynamic-light").unwrap()
    );
    ...
    assert_eq!(
        winarchy_theme::Theme::effective(&temp.0).unwrap(),
        winarchy_theme::Theme::load(&temp.0, "dynamic-light").unwrap()
    );
    shell.refresh_wallpaper();
    ...
    assert!(shell.wallpaper.is_none());
    assert_eq!(
        winarchy_theme::Theme::effective(&temp.0).unwrap().name,
        "Catppuccin Mocha"
    );
}
