...
`illium-dictate.exe` is an optional companion executable: hold a bound key,
...
Keep `illium-dictate.exe` beside `illium.exe`, then bind the one command
in `keybindings.toml`:
...
Reload the configuration. The daemon starts the resident (`illium-dictate.exe
serve`) whenever a `dictate` binding exists, and stops it with `illiumctl quit`.
Without the executable the binding logs a warning and does nothing else;
Illium itself never depends on it.

The resident starts small and loads the model only when asked: on the first
press, or through `illium-dictate.exe --load`. Loaded, it holds about 700 MB
...
running). The [Dictation applet](https://github.com/YannickHerrero/illium-applet-collection)
...
`%LOCALAPPDATA%\Illium\models\`. Nothing else is ever sent or fetched. The
...
The pill is a topmost tool window: Illium does not tile it, it never takes
...
running is ignored until it finishes. `illiumctl dictate` toggles a recording
from the command line for testing.

Diagnostics (durations and errors, never audio or text) go to `dictate.log` in
the configuration home, rotated at 1 MiB. `illium-dictate.exe --status`
