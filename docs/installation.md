...
Copy `target\release\winarchy.exe` and `winarchyctl.exe` to a directory on PATH. Defaults are embedded; the configuration directory is created on first launch. No installer, service, administrator privilege or registry change is required. The corrected daemon explicitly refuses an elevated token before opening configuration or log paths.
