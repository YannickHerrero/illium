...
fn relevant(home: &Path, path: &Path) -> bool {
    path == home.join("winarchy.toml")
        ...
        || (path.parent() == Some(home.join("themes").as_path())
            && path.extension().is_some_and(|s| s.eq_ignore_ascii_case("toml")))
}
...
pub fn watch(home: PathBuf, mut changed: impl FnMut(Theme) + Send + 'static)
    -> Result<Subscription, String>
{
    ...
    let mut watcher = notify::recommended_watcher(move |event: notify::Result<notify::Event>| {
        if event.is_ok_and(|e| !matches!(e.kind, EventKind::Access(_))
            && e.paths.iter().any(|p| relevant(&root, p))) {
            let _ = tx.try_send(());
        }
    ...
    watcher.watch(&home, RecursiveMode::Recursive).map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn only_palette_and_runtime_changes_are_relevant() {
        ...
        for p in ["terminal.log", "background-opacity.state.tmp", "themes/mine/preview.png", "state.json"] {
            assert!(!relevant(home, &home.join(p)));
        }
    }
    #[test]
    fn running_consumers_follow_override_and_reset() {
        ...
        let receive = || rx.recv_timeout(Duration::from_secs(5)).unwrap().background_opacity;
