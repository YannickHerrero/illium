//! One resident per Windows user/session, using Illium's owner-only pipe.
...
use illium_ipc::{client, identity, server};
...
pub fn run() -> Result<(), String> {
        ...
        && flag == "--demo"
    {
        let scene = match scene.as_str() {
            "build" => illium_terminal::demo::Scene::Build,
            "fetch" => illium_terminal::demo::Scene::Fetch,
    ...
    let command = match args.as_slice() {
        ...
        [arg] => match arg.as_str() {
            ...
            "--status" => "status",
            _ => {
                return Err("Usage: illium-terminal [--serve|--standalone|--status|--quit]".into());
            }
        },
        _ => return Err("Usage: illium-terminal [--serve|--standalone|--status|--quit]".into()),
    };
