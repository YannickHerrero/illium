//! Opt-in desktop probe. Start `winarchy-terminal --serve` first. Opens/closes
//! only its own new windows; never stops WSL or changes the user's theme.
#[cfg(windows)]
fn main() -> Result<(), String> {
    use std::time::{Duration, Instant};
    use winarchy_ipc::{client, identity};
    ...
    for i in 0..count {
        ...
        let at = Instant::now();
        // Same two-message path as the Winarchy hotkey, including focus grant.
        request("pid")?;
        unsafe {
