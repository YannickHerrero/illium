...
pub fn run() -> Result<(), String> {
    ...
    use model::{Action, Files, Mode, Preview};
    use slint::{ComponentHandle, ModelRc, VecModel};
    use std::{cell::RefCell, rc::Rc};
    let home = ui::config_home();
    let user = std::path::PathBuf::from(std::env::var_os("USERPROFILE").unwrap_or_default());
    ...
    ui::apply(
        ...
        &winarchy_theme::Theme::current(&home),
    );
    window.set_help_lines(ui::strings(&model::HELP));
    let files = Rc::new(RefCell::new(Files::open(start, user)));
    let render = {
    ...
    window.on_key(move |text, ctrl, _shift| {
        ...
        if changes_disk {
            f.reload();
            if let Action::Rename { to, .. } | Action::CreateDir(to) | Action::CreateFile(to) =
                &action
