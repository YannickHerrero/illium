...
fn demo_home_never_loads_the_normal_library() {
        ...
        fn drop(&mut self) {
            unsafe {
                if let Some(value) = &self.0 {
                    std::env::set_var("ILLIUM_CONFIG_HOME", value);
                } else {
                    std::env::remove_var("ILLIUM_CONFIG_HOME");
                }
            }
        }
    }
    let _restore = ConfigGuard(std::env::var_os("ILLIUM_CONFIG_HOME"));
    ...
    unsafe {
        std::env::set_var("ILLIUM_CONFIG_HOME", normal.path());
    }
...
fn tab_palette_open_filter_close_and_reopen_on_windows() {
    unsafe {
        let old_config = std::env::var_os("ILLIUM_CONFIG_HOME");
        let old_local = std::env::var_os("LOCALAPPDATA");
        let root = std::env::temp_dir().join(format!(
            "illium-palette-regression-{}-{}",
            std::process::id(),
            tabs::now()
        ));
        ...
        .unwrap();
        std::env::set_var("ILLIUM_CONFIG_HOME", &config);
        ...
        match old_config {
            Some(value) => std::env::set_var("ILLIUM_CONFIG_HOME", value),
            None => std::env::remove_var("ILLIUM_CONFIG_HOME"),
        }
        match old_local {
