...
use illium_browser::leader::{Action, CapturedKeys, Key, Leader, Outcome};
use illium_browser::tabs::{self, TabId, Tabs};
use illium_browser::{Blocker, address, library::Library};
...
fn color(s: &str) -> COLORREF {
    let (r, g, b) = illium_theme::rgb(s).unwrap();
...
unsafe fn apply_theme(theme: &illium_theme::Theme) -> AppResult<()> {
    ...
    if let Some(app) = snapshot() {
        ...
        for view in views {
            ...
            let (r, g, b) = illium_theme::rgb(&theme.background).unwrap();
    ...
    unsafe {
        ...
        let profile = if let Some(data) = demo {
            ...
            std::env::var_os("LOCALAPPDATA")
                ...
                .join("Illium/browser/profile")
        };
        std::fs::create_dir_all(&profile)?;
        let _ = SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
        let instance = HINSTANCE(GetModuleHandleW(None)?.0);
        let class = w!("IlliumBrowser");
        ...
        let _theme_subscription = illium_theme::live::watch(home.clone(), move |theme| {
                ...
                if let Some(app) = snapshot() {
                    ...
                    if let Some(tab) = active {
                        ...
                        let _ = SetWindowTextW(
                            hwnd,
                            PCWSTR(wide(&format!("{} — Illium Browser", tab.label())).as_ptr()),
                        );
                    }
                    if app.picker.borrow().visible && app.picker.borrow().tabs_mode {
                ...
                if let Some(requests) = requests {
                    while let Ok(request) = requests.try_recv() {
                        let result = if request.command == "status" {
                            ...
                            illium_browser::launch::parse_open(input).and_then(|inputs| {
                                if opened {
