...
pub fn run() -> Result<(), String> {
    ...
    if args.as_slice() == ["--demo"] {
        ...
        let data = illium_browser::demo::DemoData::new().map_err(|e| e.to_string())?;
    ...
    if args.first().is_some_and(|a| a.starts_with("--")) && !serve && !control {
        return Err(
            "Usage: illium-browser [URL ... | --serve | --standalone [URL ...] | --demo | --status | --quit]"
                .into(),
        );
    }
    let pipe = client::pipe_path(&identity::endpoint_named("illium-browser")?);
    if control {
