fn plugin(args: &[String]) -> Result<(), String> {
    use illium_config::plugins::{self, Kind};
    ...
    let backup = match args.as_slice() {
        ...
        _ => return Err("usage: illiumctl plugin list [--json] | inspect <applet|theme> <id> | install <local-folder> | update <local-folder> | enable applet <id> [--section left|center|right|drawer] | disable applet <id> | uninstall <applet|theme> <id>\nApply themes with: illiumctl theme set <id>".into()),
    };
...
fn main() {
    ...
    if args.first().is_some_and(|s| s == "theme") && args.get(1).is_some_and(|s| s == "install") {
        if args.len() != 3 {
            eprintln!("usage: illiumctl theme install <pack-folder>");
            std::process::exit(2);
        }
        match illium_theme::pack::install(
            &illium_theme::config_home(),
            std::path::Path::new(&args[2]),
        ) {
            Ok(name) => println!("installed {name}; select with: illiumctl theme set {name}"),
            Err(e) => {
                eprintln!("{e}");
                std::process::exit(1);
            }
    ...
    {
        if args.len() != 2 {
            eprintln!("usage: illiumctl lock set-password");
            std::process::exit(2);
        }
        match set_password() {
            Ok(()) => println!("lock password saved; lock with: illiumctl lock"),
            Err(e) => {
                eprintln!("{e}");
                std::process::exit(1);
            }
    ...
    if let Err(e) = command.parse::<illium_ipc::command::Command>() {
        eprintln!("{e}");
        std::process::exit(2);
    }
    #[cfg(windows)]
    match illium_ipc::client::client(&command) {
        Ok(reply) => {
            if reply.ok {
    ...
    #[cfg(not(windows))]
    {
        eprintln!("illiumctl requires Windows");
        std::process::exit(1);
    }
}
