...
pub fn same_subsystems(
    ...
    let global = home.join("illium.toml");
...
mod tests {
    ...
    impl Temp {
        fn new(name: &str) -> Self {
            let p =
                std::env::temp_dir().join(format!("illium-files-{}-{name}", std::process::id()));
    ...
    fn temporary_opacity_does_not_reload_shell_configuration() {
        ...
        std::fs::write(t.0.join("illium.toml"), "theme = 'test'").unwrap();
        let before = snapshot(&t.0).unwrap();
        illium_theme::opacity::set(&t.0, "test", 0.65).unwrap();
        assert_eq!(snapshot(&t.0).unwrap(), before);
        illium_theme::opacity::clear(&t.0).unwrap();
        assert_eq!(snapshot(&t.0).unwrap(), before);
    }
    #[test]
