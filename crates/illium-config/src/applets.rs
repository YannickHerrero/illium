...
pub struct Manifest {
    ...
    /// Passed to the provider as ILLIUM_APPLET_<KEY> environment variables.
    #[serde(default)]
...
pub fn load(home: &Path, name: &str) -> Result<Applet, String> {
    ...
    if let Some(module) = &manifest.attach
        && (!crate::config::BUILTIN_MODULES.contains(&module.as_str())
            || ["workspaces", "space", "separator"].contains(&module.as_str()))
    {
...
pub fn environment(applet: &Applet) -> Vec<(String, String)> {
    applet
        ...
        .map(|(k, v)| {
            ...
            (format!("ILLIUM_APPLET_{}", k.to_ascii_uppercase()), value)
        })
        .collect()
}
...
mod tests {
    ...
    fn disabled_direct_and_attached_applets_keep_their_configuration() {
        let home = std::env::temp_dir().join(format!("illium-disabled-{}", std::process::id()));
    ...
    fn names_and_manifests() {
        ...
        let home = std::env::temp_dir().join(format!("illium-applets-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        ...
        assert_eq!(
            environment(&a),
            vec![
                ("ILLIUM_APPLET_CITY".to_owned(), "Lyon".to_owned()),
                ("ILLIUM_APPLET_COUNT".to_owned(), "3".to_owned()),
            ]
        );
