...
fn theme_lifecycle_protects_active_theme_and_personal_wallpapers() {
    ...
    f.install();
    let original_theme = text(&f.home.join("illium.toml")).unwrap();
    assert_eq!(active_theme(&f.home).unwrap(), "catppuccin-mocha");
    fs::write(f.home.join("illium.toml"), "theme='sample'").unwrap();
    ...
    fs::write(f.home.join("illium.toml"), original_theme).unwrap();
...
#[test]
#[ignore = "requires ILLIUM_TODO_SOURCE and ILLIUM_THEME_SOURCE; only copies into temporary homes"]
fn local_collection_packages() {
    for (kind, id, variable) in [
        (Kind::Applet, "todo", "ILLIUM_TODO_SOURCE"),
        (Kind::Theme, "sample", "ILLIUM_THEME_SOURCE"),
    ] {
        let source =
