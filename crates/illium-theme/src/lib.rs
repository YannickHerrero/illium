...
pub struct Theme {
    ...
    /// Background opacity shared by Illium applications (web pages stay opaque).
    #[serde(
    ...
    /// Blur behind translucent application backgrounds. A `illium.toml`
...
pub fn config_home() -> std::path::PathBuf {
    std::env::var_os("ILLIUM_CONFIG_HOME")
...
impl Theme {
    ...
    fn global(home: &Path) -> Result<Global, String> {
        let global: Global = toml::from_str(&read(&home.join("illium.toml"))?)
    ...
    /// Name of the theme selected in `illium.toml`.
...
mod tests {
    ...
    fn selected_and_loaded_from_home() {
        let home = home();
        std::fs::write(home.join("illium.toml"), "theme = \"mine\"\n").unwrap();
        std::fs::write(
        ...
        std::fs::write(
            home.join("illium.toml"),
            "theme = \"mine\"\nbackground_blur = true\n",
        )
        .unwrap();
        assert!(Theme::current(&home).background_blur);
        assert!(Theme::parse(&format!("{DEFAULT}background_blur = true\n")).is_err());
        std::fs::write(home.join("illium.toml"), "theme = \"../x\"\n").unwrap();
    ...
    fn falls_back_without_configuration() {
        ...
        std::fs::write(home.join("illium.toml"), "theme = \"missing\"\n").unwrap();
        assert_eq!(Theme::current(&home), Theme::default_theme());
        std::fs::remove_dir_all(home).unwrap();
    }
}
