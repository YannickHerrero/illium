...
pub enum Command {
    ...
    Meta,
    /// Launch a companion application from `illium-apps.exe`: `shot`, `tasks`, `files`.
    App(String),
    Reload,
    Theme(String),
    ...
    ResetOpacity,
    /// Flip `background_blur` in `illium.toml`.
    ToggleBlur,
    ...
    BarHints,
    /// Push-to-talk dictation through `illium-dictate.exe`: bound to a held
    /// key, the daemon records while it is down; over IPC it toggles.
    Dictate,
    /// Illium's own lock screen, unlocked with the password set by
