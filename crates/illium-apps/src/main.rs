//! One executable, one subcommand per application: `illium-apps <name>`.
...
#[cfg(windows)]
mod ui;
/// Exit code for a missing or unknown subcommand, as for `illiumctl` syntax errors.
const USAGE: i32 = 2;
...
fn run(_name: &str) -> Result<(), String> {
    Err("illium-apps requires Windows".into())
}
