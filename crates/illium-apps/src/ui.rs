...
pub fn color(hex: &str) -> slint::Color {
    let (r, g, b) = illium_theme::rgb(hex).unwrap_or((255, 0, 255));
...
pub fn watch_theme<T>(window: &T) -> Result<illium_theme::live::Subscription, String>
where
    T: slint::ComponentHandle + 'static,
    for<'a> Palette<'a>: slint::Global<'a, T>,
{
    let weak = window.as_weak();
    illium_theme::live::watch(config_home(), move |theme| {
        let weak = weak.clone();
        let _ = slint::invoke_from_event_loop(move || {
            if let Some(window) = weak.upgrade() {
                ...
                if blur_changed && let Some(hwnd) = hwnd(&window) {
                    illium_theme::blur::set(hwnd.0 as isize, theme.background_blur);
                }
